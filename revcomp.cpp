...
process_block(
    ...
    size_t size =(size_t)reverse_complement(
            body_begin, body_end,
            lookup,
            buffer);
...
main() {
    ...
    f = fopen("revcomp-input25000000.txt","r+");    

    setvbuf(f, 0, _IOFBF, 1024*1024);

    long stdin_size = get_file_size(f);
    uint8_t *buffer = (uint8_t*)calloc(stdin_size + LINE_LENGTH, 1);
    assert(buffer);
    lookup_t lookup;
    pthread_mutex_t mutex;
    job_t *job = 0;
    ...
    while (fgets_unlocked((char *)current, end - current, f)) {
        if (current[0] == '>') {
            if (body_begin != 0) {
    ...
    printf("revcomp took %f seconds to execute \n", time_taken);
    return 0;
}
